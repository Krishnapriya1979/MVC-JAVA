...
public class AddressBookDAOImplementation implements AddressBookDAO{
     ...
     public void updatePerson(Person person,String name)
     {
         try
       {
           conn = DBConnection.getInstance().getConnect();
            ...
            String qry="update AddressBook set name='"+person.getName()+"',mob='"+person.getMob()+"',email='"+person.getEmail()+"','"+person.getCity()+"' where name='"+name+"'";
           st.executeUpdate(qry);
           conn.close();
        }
   ...
   @Override
    public void getSelectedName(DetailViewPanel detailPanel,String selectedName)
    {
        try {
            ...
            while(rs.next()){
             ...
             detailPanel.setCity(rs.getString("location"));
    ...
    @Override
    public DefaultListModel getSearchNames(NameListPanel namePanel,String op) { 
         DefaultListModel<String> dlm = new DefaultListModel<>();
            try {
         conn = DBConnection.getInstance().getConnect();
            Statement stmt;
             stmt = conn.createStatement();
             op = op + '%';
            String qry = "select * from AddressBook where name like '" + op + "' order by name asc";
                 try (ResultSet rs = stmt.executeQuery(qry)) {
                     while (rs.next()){
                         String name = rs.getString(1);
                         dlm.addElement(name);
